...
| `channel`   | `command/tty`        | TTY support for command channels
...
The `kind-args` field should be null or absent.
...
If both sides support the `channel=command/tty` capability, then the `meta` field may contain the following data:
...
If the `tty` field is set to true, the `meta` field should also contain the following data:

----
{
  tty-selectors: Vec<u32>,
  term: Bytes,
  modes: BTreeMap<u32, Value>,
  height-cells: u32,
  width-cells: u32,
  height-pixels: u32,
  width-pixels: u32,
...
`tty-selectors` is the list of selectors that should be connected to a TTY.
`term` is the terminal type.
`height-cells` and `width-cells` refer to the number of rows and columns, respectively, in the terminal type.
`height-pixels` and `width-pixels` refer to the number of pixels in each dimension in the terminal type.

`modes` maps terminal codes to values.
The keys are codes defined by the SSH protocol specification (including extensions) for terminal values; those greater than or equal to `0xff000000` are available to allocate with `extension=allocate`.
For flags, the value is a boolean; for characters, it is either an integer or null (if not set); and for terminal speeds, it is an integer.

If the `channel=command/tty` capability is not supported by both sides or negotiating a TTY is not desired, then `meta` should be absent or empty.
...
The `kind` field contains the notification kind as follows:

|===
| Name | Value | Meaning

| WaitStatus           | 0 | a process has exited
| TerminalWindowChange | 1 | the terminal window size for a process has changed
|===

If `kind` is `WaitStatus`, the `status-kind` field indicates a status kind as follows:
...
If the `status` code is present and non-null, it is the exit code (if the `status-kind` field is `Exited`) and the signal (if the `kind` field is `Signalled` or `SignalledWithCore`).

The `meta` field should be absent or null and is reserved for future use in this case.

If `kind` is `TerminalWindowChange`, the `status` and `status-kind` fields should be absent or null and the `meta` output should look as follows:
